...
class CatWorldScene extends Phaser.Scene {
  ...
  captureSceneDepartures(moves = [], nextSceneId = "") {
    if (
      this.isEditMode()
      || this.isToolMode()
      || nextSceneId !== this.owner.snapshot.scene.id
    ...
    return moves
      .filter((move) => catWorldSceneMoveForScene(move, this.owner.snapshot.scene.id) === "departure")
      .map((move) => {
        const entry = entries.get(move.catId);
        if (!entry?.container?.active || !entry.behavior.canWalk || this.owner.carriedCat?.catId === move.catId) {
          return null;
        }
        this.owner.catReactions.delete(move.catId);
        return {
          move,
          cat: entry.cat,
  ...
  playSceneDepartures(departures = []) {
    if (!departures.length || this.isEditMode() || this.isToolMode()) return false;
    departures.forEach((departure) => {
      const plan = catWorldSceneDeparturePlan(
        departure.move,
        departure,
        {
          minX: 38,
        ...
        departure.behavior.walkSpeed,
      );
      const ghost = this.add.container(plan.startX, plan.startY);
      ghost.setSize(100, 70);
      ghost.setData("kind", "departing-cat");
      ghost.setData("id", departure.cat.id);
      ghost.setData("behavior", departure.behavior);
      ghost.setScale(plan.facing, 1);
      ghost.setDepth(CAT_INTERACTION_DEPTH + departure.index + 90);
      this.drawCatShape(ghost, departure.cat, false, departure.snapshot, departure.behavior);
      const cue = this.add
        .text(45, -86, `去${departure.move.toSceneLabel}`, {
          color: "#263047",
        ...
        .setOrigin(0.5, 1);
      this.pinCatTextOverlay(cue);
      ghost.add(cue);
      this.syncCatTextOverlays(ghost);
      const gait = departure.behavior.gait || catWorldGaitProfile(departure.cat, departure.behavior);
      ...
      this.tweens.add({
        ...
        onUpdate: (tween) => {
          ghost.setDepth(CAT_INTERACTION_DEPTH + departure.index + 90);
          this.updateCatGait(ghost, gait, distance, tween.progress);
        },
  ...
  catDropInteraction(decorId) {
    return catDropInteractionFor(
      decorId,
      this.owner.snapshot?.gameSettings?.interactionDurations || {},
    );
  }

  autonomousCatDecorInteraction(decorId) {
    return autonomousCatDecorInteractionFor(
      decorId,
      this.owner.snapshot?.gameSettings?.interactionDurations || {},
      { lampActive: Boolean(this.owner.itemInteractionStates.get(decorId)?.active) },
    );
  }
  ...
  drawCarryTargetIndicators() {
    ...
    for (const [decorId, container] of this.decorContainers.entries()) {
      const interaction = this.catDropInteraction(decorId);
  ...
  placeCarriedCatOnDecor(entry, target) {
    ...
    if (decorId === "bubble-bathtub" && Number(this.owner.snapshot.inventory?.["cat-bath-kit"] || 0) > 0) {
      const action = {
        ...
        itemId: decorId,
        expiresAt: Date.now() + interaction.holdMs + 18000,
      };
      this.owner.catItemActions.set(entry.cat.id, action);
  ...
  startManualDecorAction(entry, action) {
    const interaction = this.catDropInteraction(action.itemId);
  ...
  startLampFavoriteAction(entry, decorId, action = null) {
    ...
    const nextAction = action || {
      kind: "lamp",
      itemId: decorId,
      expiresAt: Date.now() + interaction.holdMs + 12000,
    };
    ...
    this.moveCatForInteraction(entry, target, decorId, () => {
      ...
      this.holdCatInteraction(entry, decorId, interaction.holdMs);
    });
  }
  ...
  startDeskFavoriteInteraction(decorId) {
    ...
    movable.forEach((entry) => {
      const action = {
        kind: "desk",
        itemId: decorId,
        expiresAt: Date.now() + interaction.holdMs + 15000,
      };
      this.owner.catItemActions.set(entry.cat.id, action);
  ...
  startDeskFavoriteAction(entry, decorId, action) {
    ...
    const spec = DECOR_SPECS[decorId];
    if (!interaction || !approach || !spec) return;
        ...
        onComplete: () => {
          ...
          this.tweens.add({
            ...
            onComplete: () => {
              ...
              const holdMs = interaction.holdMs;
  ...
  restoreActiveItemInteractions() {
    ...
    for (const [catId, action] of [...this.owner.catItemActions.entries()]) {
      ...
      if (action.kind === "manual-floor") {
        this.startManualFloorAction(entry, action);
      } else if (["manual-decor", "autonomous-decor"].includes(action.kind)) {
        this.startManualDecorAction(entry, action);
    ...
    const walkTimer = this.time.delayedCall(delay, () => {
      ...
      walkTween = this.tweens.add({
        ...
        onComplete: () => {
          ...
          let interactionOwnsSchedule = false;
          if (visitPlan?.kind === "food") {
            this.spawnFoodPlayBubble(container, cat, visitPlan.target);
          } else if (visitPlan?.kind === "rest") {
            this.spawnRestBubble(container, cat, visitPlan.target.message);
            this.owner.handlers.onCatAmbient?.(cat, {
              kind: "rest-spot",
              itemId: visitPlan.target.itemId || "room-rest",
              label: visitPlan.target.label || "休息点",
            });
            ...
            if (visitPlan.target.kind === "mischief") {
              this.spawnMischiefBubble(container, cat, visitPlan.target);
            } else {
              interactionOwnsSchedule = visitPlan.target.goalKey === "favorite-decor"
                && this.startAutonomousFavoriteDecorInteraction(
                  { cat, index, container, behavior: latestBehavior },
                  visitPlan.target,
                );
              if (!interactionOwnsSchedule) this.spawnGoalBubble(container, cat, visitPlan.target);
            }
          } else if (visitPlan?.kind === "habit") {
            this.spawnIndividualHabitBubble(container, cat, visitPlan.target);
          } else if (visitPlan?.kind === "favorite") {
            interactionOwnsSchedule = this.startAutonomousFavoriteDecorInteraction(
              { cat, index, container, behavior: latestBehavior },
              visitPlan.target,
            );
            if (!interactionOwnsSchedule) this.spawnFavoritePlayBubble(container, cat, visitPlan.target);
          }
          if (!interactionOwnsSchedule) this.scheduleCatWalk(container, index, cat);
        },
      });
  ...
  playCatMicroAnimation(container, cat = {}, behavior = {}, forcedKind = "") {
    ...
    if (kind === "chirp") {
      cue.fillStyle(0xfff07d, 1);
      cue.fillRect(96, 8, 5, 5);
      cue.fillRect(105, 1, 4, 4);
      cue.fillStyle(0x87d9ff, 1);
      cue.fillRect(101, 14, 4, 4);
      cue.lineStyle(2, INK, 0.72);
      cue.lineBetween(98, 5, 94, 0);
      cue.lineBetween(106, 0, 109, -5);
      this.tweens.add({ targets: cue, x: 8, y: -12, alpha: 0, duration: 980, ease: "Cubic.easeOut", onComplete: () => cue.destroy() });
      return;
    }

    if (kind === "heart") {
      cue.fillStyle(0xff6f9f, 1);
      cue.fillRect(97, 7, 6, 6);
      cue.fillRect(105, 7, 6, 6);
      cue.fillRect(95, 12, 18, 6);
    ...
    if (kind === "book") {
      cue.fillStyle(0xfff8df, 1);
      cue.fillRect(93, 37, 11, 9);
      cue.fillRect(105, 37, 11, 9);
      cue.fillStyle(0x87d9ff, 1);
      cue.fillRect(95, 39, 7, 5);
      cue.fillStyle(0xff8cad, 1);
      cue.fillRect(107, 39, 7, 5);
      cue.lineStyle(2, INK, 1);
      cue.strokeRect(92, 36, 25, 11);
      cue.lineBetween(104, 37, 104, 46);
      this.tweens.add({ targets: cue, y: -4, yoyo: true, repeat: 1, alpha: 0.25, duration: 620, onComplete: () => cue.destroy() });
      return;
    }

    if (kind === "lookout") {
      cue.fillStyle(0x87d9ff, 1);
      cue.fillRect(99, 4, 5, 5);
      cue.fillRect(110, -3, 7, 7);
      cue.fillStyle(0xfff07d, 1);
      cue.fillRect(119, 9, 4, 4);
      cue.lineStyle(2, INK, 0.7);
      cue.lineBetween(102, 2, 107, -4);
      cue.lineBetween(115, 4, 120, 0);
      this.tweens.add({ targets: cue, x: 7, alpha: 0, duration: 1050, ease: "Sine.easeOut", onComplete: () => cue.destroy() });
      return;
    }

    if (kind === "paw") {
      cue.fillStyle(0xff8cad, 1);
      cue.fillRect(97, 34, 8, 7);
      cue.fillRect(94, 28, 4, 4);
      cue.fillRect(101, 25, 4, 4);
      cue.fillRect(108, 28, 4, 4);
      cue.lineStyle(2, INK, 0.6);
      cue.strokeRect(97, 34, 8, 7);
      this.tweens.add({ targets: cue, x: 12, y: -8, alpha: 0, duration: 900, ease: "Cubic.easeOut", onComplete: () => cue.destroy() });
      return;
    }
    ...
    if (kind === "hop") {
      ...
      this.tweens.add({
        ...
        ease: "Quad.easeOut",
        onComplete: () => {
          if (body.active) body.setPosition(0, 0);
        },
      });
      return;
    }
  ...
  catIdleMessage(cat = {}, behavior = {}) {
    ...
    const habitThought = String(cat.individualHabit?.thought || "").trim();
    if (habitThought && Phaser.Math.Between(1, 100) <= 24) return habitThought;
  ...
  individualHabitTarget(cat = {}, index = 0, behavior = {}) {
    ...
    const targetItemIds = Array.isArray(habit.targetItemIds) ? habit.targetItemIds : [];
    const availableTargets = targetItemIds
    ...
    const selectedIndex = Math.abs(hashText(`${cat.id || "cat"}:${habit.behaviorKey || "habit"}:${hour}`)) % availableTargets.length;
    const selected = availableTargets[selectedIndex];
    const curiosity = Number(behavior.curiosity || 50);
    const mood = Number(behavior.mood || 50);
    const moodBonus = mood >= 72 ? 6 : mood < 38 && habit.behaviorKey === "quiet-retreat" ? 18 : 0;
    return {
      kind: "individual-habit",
      itemId: selected.itemId,
      itemKind: selected.point.itemKind,
      label: selected.point.label || "习惯地点",
      message: String(habit.thought || "").trim(),
      animation: String(habit.animation || "blink"),
      priority: clamp(Number(habit.priority || 48) + Math.round((curiosity - 50) / 7) + moodBonus, 28, 78),
      x: clamp(selected.point.x + seededOffset(`${cat.id}:${selected.itemId}:habit-x`, 24), 38, GAME_WIDTH - 132),
      y: clamp(selected.point.y + seededOffset(`${cat.id}:${selected.itemId}:habit-y`, 12), FLOOR_TOP + 52, FLOOR_BOTTOM - 70),
    };
  }
  ...
  spawnIndividualHabitBubble(container, cat, target = {}) {
    if (!container?.active) return;
    const catLabel = cat?.displayLabel || cat?.label || "猫咪";
    const message = `${catLabel}：${target.message || `我想在${target.label || "这里"}待一会儿。`}`;
    const bubble = this.add
      ...
      .setOrigin(0.5)
      .setDepth(CAT_INTERACTION_DEPTH + 132);
    this.playCatMicroAnimation(container, cat, container.getData("behavior") || {}, target.animation);
    this.tweens.add({
      targets: bubble,
      y: bubble.y - 18,
      alpha: 0,
  ...
  decorOccupiedByAnotherCat(itemId, catId = "") {
    for (const [activeCatId, action] of this.owner.catItemActions.entries()) {
  ...
  startAutonomousFavoriteDecorInteraction(entry, target = {}) {
    const itemId = String(target.itemId || target.decorId || "");
    if (target.itemKind !== "decor" || !itemId) return false;
    const interaction = this.autonomousCatDecorInteraction(itemId);
    const spec = DECOR_SPECS[itemId];
    const decor = this.decorContainers.get(itemId);
    if (!interaction || !spec || !decor?.active || decor.getData("damaged")) return false;
    const action = {
