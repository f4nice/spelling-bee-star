...
- 2026-09-24：按用户要求将中秋/国庆学习能量每日额外加赠上限从 500 提高到 1500；前台提示及活动规则统一读取服务端上限。回归覆盖超过原 500 上限仍可加赠、1500 封顶、跨日和活动结束保留。发布 `BIZ-REL-20260924-002`，更新猫咪世界模块。

- 2026-09-24：中秋（9/25–27）与国庆（10/1–7）按北京时间对新获得的拼写挑战、整轮挑战、作文提高积分、AI Debate 学习能量加赠 100%，每账号每日额外最多 500。新增事件流水防重记账，奖励跨日/活动结束保留；猫咪世界展示预告、今日进度与规则。两期节庆盲盒各 1500 能量、各限开一次，每期两款节日猫咪各 50%，按服务端活动时间开放；仅限时间、不设全站数量上限。活动室增加铲子快捷开关并连续清理，库存耗尽自动收起。发布号 `BIZ-REL-20260924-001`。Grok 参与铲屎交互局部审查，实际输入 6415、输出 1075、合计 7490 tokens。验证：107 项相关 Python 测试、259 项前端测试及本地真实页面清理三堆、库存耗尽、盲盒预告显示检查。
