...
- 2026-09-24（Grok 开发工具，尚未接通）：已读取本机配置，首次小请求及模型列表查询返回 HTTP 400；诊断识别为认证失败，本地格式检查也未通过。未取得模型输出或用量数据，需要用户重新在本机输入完整 xAI 密钥。补充保存前格式校验、凭据来源提示、只读模型查询和不泄露上游错误正文的诊断；网站未发布。
